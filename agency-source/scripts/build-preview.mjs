...
const {render, siteTitle, siteDescription} = await import(pathToFileURL(path.join(root, 'out/render/render.mjs')).href);
...
const doc = (page, offline = false) => `<!doctype html><html lang="en" data-preview="${offline ? 'offline' : 'static'}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${page === 'home' ? siteTitle : `${page === '404' ? 'Page not found' : page.charAt(0).toUpperCase()+page.slice(1).replaceAll('-',' ')} | Service Capture Co.`}</title><meta name="description" content="${siteDescription}"><link rel="icon" href="data:image/svg+xml,${encodeURIComponent(favicon)}">${offline ? `<style>${css.replaceAll('</style','<\\/style')}</style>` : '<link rel="stylesheet" href="/assets/site.css">'}</head><body><div id="root">${render(page, offline)}</div>${offline ? `<script id="everwarm-data" type="application/octet-stream">${demo.toString('base64')}</script><script>${js.replaceAll('</script','<\\/script')}</script>` : '<script src="/assets/site.js" defer></script>'}</body></html>`;
